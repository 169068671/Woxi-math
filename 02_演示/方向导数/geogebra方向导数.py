...
# 若不在 venv 里，尝试自动切换（venv 在项目根目录，即脚本上两级）
def _find_venv_python(start: Path) -> Path | None:
    for p in [start, *start.parents]:
        candidate = p / "venv" / "bin" / "python3.14"
        if candidate.exists():
            return candidate
    return None

_VENV_PY = _find_venv_python(Path(__file__).resolve().parent)
